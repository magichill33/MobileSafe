...
public class QuestionHistoryDisposeActivity extends NormalBaseActivity
	...
	private HistoryDisposeBean historyDisposeBean = QuestionHistoryDisposeListActivity.bean;
	...
	protected void installViews() {
		// TODO Auto-generated method stub
		setContentView(R.layout.activity_question_history_dispose);
		initTitleBar();
		initValues();
	...
	protected void initTitleBar() {
		...
		mTitleBar.centerTextView.setText(R.string.function_history_bl_i);
	...
	private void readData() {
		...
		for (int i = 0; i < historyDisposeBean.getList().size() / 2; i++) {
			Map<String, Object> map = new HashMap<String, Object>();
			map.put(LayoutStructure.from[0],
					historyDisposeBean.getList().get(i * 2));
			map.put(LayoutStructure.from[1],
					historyDisposeBean.getList().get(i * 2 + 1));
			data.add(map);
		}

		// 图片写入
		if (null != (imageList1 = historyDisposeBean.getApproveFileList())) {
			for (int i = 0; i < imageList1.size(); i++) {
				ImageTool.writeFile(imageList1.get(i));
			}
		}
		if (null != (imageList2 = historyDisposeBean.getDeptFileList())) {
