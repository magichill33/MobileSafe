...
public class MapSetupActivity extends NormalBaseActivity implements
		OnClickListener {
	...
	private void initValues() {
		String defultUrl = new SharedPreferencesFactory()
				.getMapConfig(mContext);
		if (null != defultUrl) {
			String strs[] = defultUrl.split(",");
			setup_ip.setText(strs[0] + "");
			setup_point.setText(strs[1] + "");
		} else {
			defultUrl = Config.getMapURL_();
			...
			new SharedPreferencesFactory().changeMapIpConfig(
					mContext,
	...
	public void onClick(View arg0) {
		...
		case R.id.setup_submit:
			new SharedPreferencesFactory().changeMapIpConfig(mContext, setup_ip
					.getText().toString(), setup_point.getText().toString());
			Toast.makeText(mContext, "地图IP修改成功！", Toast.LENGTH_SHORT).show();
			finish();
			break;
